...
- an object-aware shell engine that recovers indirect objects, xref/trailer structure, repair state, page-resolution provenance, and content-stream provenance for observed text, with operator-ready stream bodies for unfiltered and `FlateDecode` streams, document-specific shell limitation markers, runtime support claims, and a no-op disposal contract for future worker or WASM backends
