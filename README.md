...
It does not yet ship a finished parser, a mature layout engine, mature structured knowledge projections, a pixel-accurate renderer, or benchmark-backed superiority claims. The current render stage now exposes deterministic text indexing, selection geometry, resource payload references, page imagery, and deterministic raster output, but it still carries truthful partial-imagery limits and the viewer does not yet consume render artifacts directly.
