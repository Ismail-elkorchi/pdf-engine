...
`npm run test:coverage:node-lower` reports line, branch, and function coverage for the Node lower-layer suites. Treat those numbers as a debugging signal for missing tests, not as proof that the changed subsystem is correct or complete.
...
- Do not claim a subsystem is done from coverage percentages alone; explain which invariants, relations, or cross-runtime proofs support the change.
